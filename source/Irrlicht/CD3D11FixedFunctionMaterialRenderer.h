...
public:
	...
	CD3D11FixedFunctionMaterialRenderer(ID3D11Device* device, IVideoDriver* driver)
	: CD3D11MaterialRenderer(device, driver, NULL)
	...
	, Sampler1(0), Sampler2(0)
	, Texture1(0), Texture2(0)
	{
		IMaterialRenderer* solidRend = Driver->getMaterialRenderer(EMT_SOLID);
		if (solidRend)
		{
			Effect = static_cast<CD3D11FixedFunctionMaterialRenderer*>(solidRend)->Effect;
	...
	virtual bool OnRender(IMaterialRendererServices* service, E_VERTEX_TYPE vtxtype)
	{
		...
		// Apply pass
		CurrentPass->Apply( 0, ImmediateContext);
...
protected:
	...
	bool init()
	{
		...
		hr = D3DCompile( FIXED_FUNCTION_SHADER, strlen(FIXED_FUNCTION_SHADER), "", NULL, NULL, NULL, "fx_5_0", flags, 0, &ppCode, &ppErrors );
...
public:
	...
	: CD3D11FixedFunctionMaterialRenderer(device, driver) {}
...
public:
					...
					bool resetAllRenderstates, IMaterialRendererServices* services)
	{
		...
		// set texture matrix
		setVariable("TextureMatrix0", reinterpret_cast<f32*>(&SphereMapMatrixD3D11), sizeof(SphereMapMatrixD3D11));
	}

	virtual void OnUnsetMaterial()
	{
		setVariable("TextureMatrix0", reinterpret_cast<f32*>(&UnitMatrixD3D11), sizeof(UnitMatrixD3D11));
	}	
};
...
public:
					...
					bool resetAllRenderstates, IMaterialRendererServices* services)
	{
		...
		D3D11_DEPTH_STENCIL_DESC& depth = Driver->getDepthStencilDesc();
