...
public:
	...
	<tt>AutomaticCullingEnabled = false;<tt>.
	*/
	...
	*/
	...
	return the only one material, assuming that no one ever calls getMaterial()
...
int main()
{
	...
	*/
	...
	if(anim)
	{
		...
		/*
		I'm done referring to anim, so must drop this reference now
		because it was produced by a createFoo() function. As I
		shouldn't refer to it again, ensure that I can't by setting to 0.
		*/
		anim->drop();
		anim = 0;
	}
	...
	custom scene node is removed from the scene graph.
	*/
	myNode->drop();
